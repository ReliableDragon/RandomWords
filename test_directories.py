...
class TestDirectories():
  ...
  def __enter__(self):
    ...
    self.d3 = TemporaryDirectory(dir=self.d2.name)
    self.d4 = TemporaryDirectory(dir=self.d2.name)

    self.tf1 = NamedTemporaryFile(suffix='.txt', dir=self.d1.name)
    self.tf2 = NamedTemporaryFile(suffix='.txt', dir=self.d2.name)
    ...
    self.txts = [self.tf1.name, self.tf2.name, self.tf3.name, self.tf4.name]
